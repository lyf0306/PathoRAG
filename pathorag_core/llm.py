...
async def openai_complete_if_cache(
    ...
    **kwargs,
    ...
    inc_counter("llm_calls_total", labels={"provider": "openai"})
    t_start = time.time()
    try:
        if api_key:
        ...
        if hasattr(response, "__aiter__"):
            ...
            observe_histogram("llm_call_duration_seconds", time.time() - t_start, labels={"provider": "openai"})
            return inner()
        else:
        ...
        inc_counter("llm_errors_total", labels={"provider": "openai"})
        raise
...
async def openai_embedding(
    ...
    inc_counter("embedding_calls_total", labels={"provider": "openai"})
    t_start = time.time()
    try:
        if api_key:
            os.environ["OPENAI_API_KEY"] = api_key
        ...
        observe_histogram("embedding_duration_seconds", time.time() - t_start, labels={"provider": "openai"})
        return result
    except Exception:
        inc_counter("embedding_errors_total", labels={"provider": "openai"})
        raise
