...
from pathorag_core.prompt import PROMPTS
...
class OverloadError(Exception):
    """Raised when the system rejects a request due to capacity limits.

    Callers should retry with exponential backoff or return HTTP 503.
    """


ENCODER = None

logger = logging.getLogger("pathorag_core")
...
def limit_async_func_call(max_size: int, waitting_time: float = 0.0001):
    ...
    不再使用自旋锁(while sleep)，彻底解决高并发下的死锁和CPU空转问题。
    """
